...
public class ThreadPoolSizeModifier implements Runnable {
    ...
    private final ArrayList<Memory> metricMemory;
    /*
     * Constructor
     *
    ...
    public void run() {
        try {
            if (!HAS_STARTED && (LATENCY_TIMER.getCount() > 0)) { // When this method is executed for the first
                // time
                INC_ITR = true;
                HAS_STARTED = true;
            }
            ...
            if (optimizationAlgorithm.equals("T")) { // If Throughput Optimized
                if ((!DEC_ITR || !DEC_IMPROVED) && (oldTenSecondRate - currentTenSecondRate) / oldTenSecondRate * 100 > 10 && resetMemory != 300) {
                    System.out.println("LOW");
                    Memory current;
                    for (Memory memory : metricMemory) {
                        current = memory;
                ...
                if (INC_ITR && INC_IMPROVED) {
                    System.out.println("ITR1");
                    threadPool.incrementPoolSizeBy(10);
                }
                if (INC_CHECK_ITR && INC_IMPROVED) {
            ...
            if (optimizationAlgorithm.equals("M")) { // If Mean latency Optimized
                ...
                if (DEC_ITR && DEC_IMPROVED) {
                    threadPool.decrementPoolSizeBy(10);
                }
                if (DEC_CHECK_ITR && DEC_IMPROVED) {
                    System.out.println((((oldMeanLatency - currentMeanLatency) / oldMeanLatency) * 100));
                    if ((((oldMeanLatency - currentMeanLatency) / oldMeanLatency) * 100) < 5) {
                        DEC_IMPROVED = false;
                        threadPool.incrementPoolSizeBy(10);
            ...
            if (INC_ITR) { // If current iteration is INC_ITR, next would be INC_CHECK_ITR
                INC_ITR = false;
                INC_CHECK_ITR = true;
                DEC_ITR = false;
                DEC_CHECK_ITR = false;
            } else if (INC_CHECK_ITR) { // If current iteration is INC_CHECK_ITR, next would be DEC_ITR
                ...
                INC_CHECK_ITR = false;
                DEC_CHECK_ITR = false;
            } else if (DEC_ITR) { // If current iteration is DEC_ITR, next would be DEC_CHECK_ITR.
                // Throughput optimization would never reach this if condition
                DEC_ITR = false;
                DEC_CHECK_ITR = true;
            } else if (DEC_CHECK_ITR) { // If current iteration is DEC_CHECK_ITR, next would be INC_ITR.
                // Throughput optimization would never reach this if condition
                INC_ITR = true;
                INC_CHECK_ITR = false;
                DEC_ITR = false;
